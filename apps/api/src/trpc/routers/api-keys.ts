...
import { apiKeyCache } from "@vision_dashboard/cache/api-key-cache";
import {
  deleteApiKey,
  getApiKeysByTeam,
  upsertApiKey,
} from "@vision_dashboard/db/queries";
import { ApiKeyCreatedEmail } from "@vision_dashboard/email/emails/api-key-created";
    ...
    .mutation(async ({ ctx: { db, teamId, session, geo }, input }) => {
      ...
      if (data) {
        try {
          ...
          resend.emails.send({
            from: "Middaybot <middaybot@vision_dashboard.ai>",
